...
export def is-valid-transition [from_state: string, to_state: string]: nothing -> bool {
  let allowed = ($VALID_TRANSITIONS | get --optional $from_state | default [])
  $to_state in $allowed
}

# Transition job to new state with validation
export def transition-state [job_id: string, new_state: string, reason: string] {
  let jid = (validate-ident $job_id "transition-state.job_id")
  ...
  let old_state = $current.0.status

  # Validate transition
  if not (is-valid-transition $old_state $new_state) {
    error make {
      msg: $"Invalid state transition: ($old_state) → ($new_state)"
      label: {
        text: $"Valid transitions from ($old_state): (($VALID_TRANSITIONS | get --optional $old_state | default []) | str join ', ')"
      }
    }
  }
  ...
  match $new_state {
    "scheduled" => {
      # Should already have scheduled_start_at set
      sql-exec $"UPDATE jobs SET status = 'scheduled' WHERE id = '($jid)'"
    }
    "ready" => {
      sql-exec $"UPDATE jobs SET status = 'ready' WHERE id = '($jid)'"
    }
    "running" => {
      sql-exec $"UPDATE jobs SET status = 'running', started_at = COALESCE\(started_at, datetime\('now'\)\) WHERE id = '($jid)'"
    }
    "suspended" => {
      sql-exec $"UPDATE jobs SET status = 'suspended' WHERE id = '($jid)'"
    }
    "backing-off" => {
      # Increment retry count and calculate next retry time
      let retry_info = (sql $"SELECT retry_count FROM jobs WHERE id = '($jid)'").0
      let new_count = ($retry_info.retry_count | default 0) + 1
      # Default retry: 1 second, exponential backoff with scaling factor 2
      let delay_sec = 1 * (2 ** ($new_count - 1))
      let reason_escaped = (sql-escape-text $reason)
      sql-exec $"UPDATE jobs SET status = 'backing-off', next_retry_at = datetime\('now', '+($delay_sec) seconds'\), retry_count = ($new_count), last_failure = '($reason_escaped)' WHERE id = '($jid)'"
    }
    "paused" => {
      sql-exec $"UPDATE jobs SET status = 'paused' WHERE id = '($jid)'"
    }
    "completed" => {
      # Determine if success or failure
      let completion_result = if ($reason | str starts-with "failure") { "failure" } else { "success" }
      let completion_failure = if $completion_result == "failure" { ($reason | str replace "failure: " "") } else { "" }
      sql-exec $"UPDATE jobs SET status = 'completed', completed_at = datetime('now'), completion_result = '($completion_result)', completion_failure = '(sql-escape-text $completion_failure)' WHERE id = '($jid)'"
    }
    _ => {
      error make { msg: $"Unknown target state: ($new_state)" }
    }
  }

  # Emit event
  emit-event $jid "" "job.StateChange" $old_state $new_state (sql-escape-text $reason)
}

# Calculate next retry timestamp with exponential backoff
export def calc-next-retry-at [base_delay_sec: int, scaling: int, attempt: int]: nothing -> string {
  let delay_sec = $base_delay_sec * ($scaling ** ($attempt - 1))
  sqlite3 $"SELECT datetime\('now', '+($delay_sec) seconds'\) as next_retry" | get next_retry.0
}

# Job pickup: acknowledge job and transition pending → ready
export def job-pickup [job_id: string] {
  let jid = (validate-ident $job_id "job-pickup.job_id")
  transition-state $jid "ready" "processor acknowledged"
}

# Job scheduler poll: find scheduled jobs ready to run
export def job-scheduler-poll []: nothing -> list<string> {
  let ready_jobs = (sql "SELECT id FROM jobs WHERE status = 'scheduled' AND scheduled_start_at <= datetime('now')")
  let job_ids = ($ready_jobs | get id | default [])
  for job_id in $job_ids {
    try {
      transition-state $job_id "ready" "scheduled time reached"
    } catch {|e|
      # Log error but continue processing other jobs
      print $"Error transitioning job ($job_id): ($e.msg)"
    }
  }
  $job_ids
}

# Job retry poll: find backing-off jobs ready to retry
export def job-retry-poll []: nothing -> list<string> {
  let ready_jobs = (sql "SELECT id FROM jobs WHERE status = 'backing-off' AND next_retry_at <= datetime('now')")
  let job_ids = ($ready_jobs | get id | default [])
  for job_id in $job_ids {
    try {
      transition-state $job_id "running" "retry time reached"
    } catch {|e|
      # Log error but continue processing other jobs
      print $"Error transitioning job ($job_id): ($e.msg)"
    }
