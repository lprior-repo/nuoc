...
export def db-init [] {
  ...
  sqlite3 $DB_PATH "
    ...
    CREATE INDEX IF NOT EXISTS idx_task_queues_queue_status ON task_queues(queue_name, status);
    CREATE INDEX IF NOT EXISTS idx_task_queues_claimed ON task_queues(claimed_by, status);
...
# Register a worker for specific task queues
# Precondition: worker_id is a valid identifier, queues is non-empty list
# Postcondition: worker record created with specified queues and slot configuration
export def worker-register [
  worker_id: string,
  queues: list<string>,
  --max-slots: int = 5
]: nothing -> nothing {
  let wid = (validate-ident $worker_id "worker-register.worker_id")

  # Validate queues
  if ($queues | is-empty) {
    error make { msg: "Worker must register for at least one queue" }
  }

  # Serialize queues as JSON array
  let queues_json = ($queues | to json -r)
  let queues_esc = (sql-escape-text $queues_json)

  # Insert worker record
  sql-exec $"INSERT INTO workers \(id, queues, max_slots\) VALUES \('($wid)', '($queues_esc)', ($max_slots)\)"
}

# Update worker heartbeat timestamp
# Precondition: worker_id exists in workers table
# Postcondition: last_heartbeat updated to current time
export def worker-heartbeat [worker_id: string]: nothing -> nothing {
  let wid = (validate-ident $worker_id "worker-heartbeat.worker_id")
  sql-exec $"UPDATE workers SET last_heartbeat = datetime\('now'\) WHERE id = '($wid)'"
}

# Unregister a worker (graceful shutdown)
# Precondition: worker_id exists in workers table
# Postcondition: worker record removed from database
export def worker-unregister [worker_id: string]: nothing -> nothing {
  let wid = (validate-ident $worker_id "worker-unregister.worker_id")
  sql-exec $"DELETE FROM workers WHERE id = '($wid)'"
}

# Enqueue a task to the appropriate queue
# Precondition: job_id and task_name are valid identifiers
# Postcondition: task record created in task_queues table with QUEUED status
export def "task-enqueue" [
  job_id: string,
  task_name: string,
  --agent-type: string
]: nothing -> nothing {
  let jid = (validate-ident $job_id "task-enqueue.job_id")
  let tname = (validate-ident $task_name "task-enqueue.task_name")

  # Determine queue name based on task type
  let queue_name = $"agent:($agent_type)"

  # Insert into task queue (UNIQUE constraint prevents duplicate enqueue)
  sql-exec $"INSERT OR IGNORE INTO task_queues \(job_id, task_name, queue_name\) VALUES \('($jid)', '($tname)', '($queue_name)'\)"
}

# Worker poll for and claim a task from a queue
# Precondition: worker_id exists and has available slots
# Postcondition: returns claimed task record or null if queue empty or worker at capacity
export def worker-poll [worker_id: string, queue_name: string] {
  let wid = (validate-ident $worker_id "worker-poll.worker_id")
  let qname_esc = (sql-escape-text $queue_name)

  # Check if worker has available slots
  let worker = (sql $"SELECT id, max_slots, active_slots FROM workers WHERE id = '($wid)'")
  if ($worker | is-empty) {
    error make { msg: $"Worker not found: ($wid)" }
  }

  let max_slots = $worker.0.max_slots
  let active_slots = $worker.0.active_slots
  ...
  # Atomically claim a task using transaction
  # First, find a QUEUED task
  let task = (sql $"SELECT id, job_id, task_name FROM task_queues WHERE queue_name = '($qname_esc)' AND status = 'QUEUED' LIMIT 1")
  ...
  let task_name = $task.0.task_name

  # Claim the task atomically
  sql-exec $"UPDATE task_queues SET status = 'CLAIMED', claimed_by = '($wid)', claimed_at = datetime\('now'\), heartbeat_at = datetime\('now'\) WHERE id = ($task_id) AND status = 'QUEUED'"

  # Increment worker's active slot count
  sql-exec $"UPDATE workers SET active_slots = active_slots + 1 WHERE id = '($wid)'"

  # Return the claimed task
  sql $"SELECT id, job_id, task_name, queue_name, status, claimed_by, claimed_at FROM task_queues WHERE id = ($task_id)" | get 0?
}

# Update heartbeat for a claimed task
# Precondition: task is claimed by worker_id
# Postcondition: heartbeat_at updated to current time
export def task-heartbeat [job_id: string, task_name: string, worker_id: string]: nothing -> nothing {
  let jid = (validate-ident $job_id "task-heartbeat.job_id")
  let tname = (validate-ident $task_name "task-heartbeat.task_name")
  let wid = (validate-ident $worker_id "task-heartbeat.worker_id")

  # Update both task queue and worker heartbeat
  sql-exec $"UPDATE task_queues SET heartbeat_at = datetime\('now'\) WHERE job_id = '($jid)' AND task_name = '($tname)' AND claimed_by = '($wid)'"
  sql-exec $"UPDATE workers SET last_heartbeat = datetime\('now'\) WHERE id = '($wid)'"
}

# Release a task (completion or failure)
# Precondition: task is claimed by worker_id
# Postcondition: task removed from queue, worker slot decremented
export def task-release [job_id: string, task_name: string, worker_id: string]: nothing -> nothing {
  let jid = (validate-ident $job_id "task-release.job_id")
  let tname = (validate-ident $task_name "task-release.task_name")
  let wid = (validate-ident $worker_id "task-release.worker_id")

  # Remove from queue
  sql-exec $"DELETE FROM task_queues WHERE job_id = '($jid)' AND task_name = '($tname)' AND claimed_by = '($wid)'"

  # Decrement worker's active slot count
  sql-exec $"UPDATE workers SET active_slots = active_slots - 1 WHERE id = '($wid)' AND active_slots > 0"
}

# Run reaper to reclaim tasks from dead workers
# Precondition: none
# Postcondition: tasks with stale heartbeats are re-enqueued, worker slots decremented
export def reaper-run [--timeout-sec: int = 30]: nothing -> nothing {
  let timeout_sec = $timeout_sec

  # Find all CLAIMED tasks with stale heartbeats
  let stale_tasks = (sql $"SELECT id, job_id, task_name, claimed_by FROM task_queues WHERE status = 'CLAIMED' AND datetime\('now'\) - datetime\(heartbeat_at\) > ($timeout_sec) || 'seconds'")

  for task in $stale_tasks {
    let task_id = $task.id
    let worker_id = $task.claimed_by

    # Re-enqueue the task (reset claim)
    sql-exec $"UPDATE task_queues SET status = 'QUEUED', claimed_by = NULL, claimed_at = NULL, heartbeat_at = NULL WHERE id = ($task_id)"

    # Decrement worker's active slot count
    sql-exec $"UPDATE workers SET active_slots = active_slots - 1 WHERE id = '($worker_id)' AND active_slots > 0"
  }
}

# Get current queue depth for a specific queue
# Precondition: queue_name is valid
# Postcondition: returns count of QUEUED tasks in queue
export def queue-depth [queue_name: string] {
  # Queue names can contain colons (e.g., "agent:general-purpose"), so escape them for SQL
  let qname_esc = (sql-escape-text $queue_name)

  let result = (sql $"SELECT COUNT\(1\) as count FROM task_queues WHERE queue_name = '($qname_esc)' AND status = 'QUEUED'")
  if ($result | is-empty) {
    0
  } else {
    $result.0.count | into int
  }
}

# Get all workers and their status
# Precondition: none
# Postcondition: returns table of all workers with slot counts
export def worker-list []: nothing -> table {
  sql "SELECT id, queues, max_slots, active_slots, last_heartbeat, registered_at FROM workers ORDER BY registered_at"
}

# Get all queued tasks across all queues
# Precondition: none
# Postcondition: returns table of queued tasks
export def task-queue-list [--queue: string] {
  if ($queue | is-empty) {
    sql "SELECT id, job_id, task_name, queue_name, status, claimed_by, enqueued_at FROM task_queues ORDER BY enqueued_at"
  } else {
    let qname_esc = (sql-escape-text $queue)
    sql $"SELECT id, job_id, task_name, queue_name, status, claimed_by, enqueued_at FROM task_queues WHERE queue_name = '($qname_esc)' ORDER BY enqueued_at"
  }
}
